...
import {
  ...
  SYNC_KEY_DERIVATION,
  SYNC_SCHEMA_VERSION,
  type SyncPushAccepted,
  type SyncPushResult,
  type SyncRecord,
  type WrappedVaultKey,
} from '@/shared/sync'

export const redirectUri = 'klarkey://oauth/callback'
...
const isOptionalDeletedAt = (value: unknown) =>
  value === undefined || isSafeIntegerInRange(value, 1)
...
export function normalizeBootstrapResult(value: unknown): BootstrapResult {
  ...
  if (response.wrappedVaultKey !== undefined && !isWrappedVaultKey(response.wrappedVaultKey)) {
...
export function normalizePullResult(value: unknown): PullResult {
  ...
  if (
    ...
    response.records.length > maxPullRecords
  ) {
  ...
  if (!response.records.every(isSyncRecord)) {
...
export function normalizePushResult(value: unknown): SyncPushResult {
  ...
  if (
    ...
    response.accepted.length > maxPushBatchRecords ||
    response.conflicts.length > maxPushBatchRecords
  ) {
    throw new Error('Invalid Klarkey sync push response.')
  }
  if (!response.accepted.every(isPushAccepted) || !response.conflicts.every(isSyncRecord)) {
