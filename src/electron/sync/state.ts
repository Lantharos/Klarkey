...
import type { SyncPushAccepted } from '@/shared/sync'
...
export function getRecordState(db: Database.Database, recordId: string): SyncRecordState | undefined {
  const row = db.prepare('SELECT recordId, revision, contentHash, serverSequence, deletedAt FROM sync_record_state WHERE recordId = ?').get(recordId) as
    | {
        recordId: string
        revision: number
        contentHash: string
...
export function listRecordStates(db: Database.Database): SyncRecordState[] {
  return db.prepare('SELECT recordId, revision, contentHash, serverSequence, deletedAt FROM sync_record_state').all() as SyncRecordState[]
}

export function upsertRecordState(db: Database.Database, state: SyncRecordState) {
  db.prepare(`
    INSERT INTO sync_record_state(recordId, revision, contentHash, serverSequence, updatedAt, deletedAt)
    VALUES (?, ?, ?, ?, ?, ?)
  ...
  `).run(state.recordId, state.revision, state.contentHash, state.serverSequence, now(), state.deletedAt ?? null)
}

export function acceptPushedRecords(db: Database.Database, records: SyncPushAccepted[], deletedAtByRecordId = new Map<string, number | undefined>()) {
