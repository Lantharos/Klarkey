...
import { copySecret } from '@/desktop/clipboard'
import { clearDeleteMarkers, deletionMap, markItemDeleted } from '@/desktop/delete-tombstones'
import { createDevicePasskeyApi } from '@/desktop/device-passkeys'
import { exportCsv, exportJson } from '@/desktop/export-vault'
import { parseOnePux } from '@/desktop/import-1pux'
import { parseVaultArchive, parseVaultImport } from '@/desktop/import-vault'
import { lockedResult } from '@/desktop/locked-result'
import { createSecretHash, shouldUpgradeSecretHash, type StoredSecretHash, verifySecretHash } from '@/desktop/secret-hash'
import { mergeNativeState } from '@/desktop/state-merge'
import { bindSyncNormalizer, createDesktopSync, type DesktopSyncState } from '@/desktop/sync'
import { deleteSystemUnlock, ensureSystemUnlock, startSystemUnlockRefresh, systemUnlockAvailable, systemUnlockSafeStorageAvailable } from '@/desktop/system-unlock'
import { checkUnlockAttempt, recordUnlockFailure, resetUnlockRateLimit } from '@/desktop/unlock-rate-limit'
import { vaultSnapshot } from '@/desktop/vault-snapshot'
...
type LaunchOnStartupRuntime = {
  isEnabled: (fallback: boolean) => Promise<boolean>
  setEnabled: (enabled: boolean) => Promise<boolean>
}

type LocalVaultRuntimeOptions = {
  launchOnStartup?: LaunchOnStartupRuntime
}

const storageKey = 'klarkey.desktop.vault.v1'
...
function publishLockState() {
  window.dispatchEvent(new CustomEvent<VaultLockInfo>('klarkey-desktop-lock-state', { detail: lockState() }))
}
...
async function resolveLaunchOnStartup(fallback: boolean, runtime?: LaunchOnStartupRuntime) {
  if (!runtime) return fallback
  try {
    return await runtime.isEnabled(fallback)
  } catch {
    return fallback
  }
}

async function setLaunchOnStartup(enabled: boolean, runtime?: LaunchOnStartupRuntime) {
  if (!runtime) return false
  try {
    return await runtime.setEnabled(enabled)
  } catch {
    return false
  }
}
...
export function createLocalVaultApi(
  nativeCall: <Result>(command: string, args?: Record<string, unknown>) => Promise<Result>,
  options: LocalVaultRuntimeOptions = {},
): KlarkeyApi {
  ...
  const syncApi = createDesktopSync({ nativeCall, loadState, saveState, requireUnlocked, normalizeItem, id, now })
  ...
  const api: KlarkeyApi = {
    ...
    settings: {
      get: async () => {
        ...
        return {
          ...current,
          launchOnStartup: await resolveLaunchOnStartup(current.launchOnStartup, options.launchOnStartup),
        }
      },
      set: async (update: SettingsUpdate) => {
        ...
        if (update.launchOnStartup !== undefined) {
          normalizedUpdate.launchOnStartup = await setLaunchOnStartup(update.launchOnStartup, options.launchOnStartup)
        }
    ...
    onLockStateChanged: (callback) => {
      ...
      return () => window.removeEventListener('klarkey-desktop-lock-state', handler)
    },
