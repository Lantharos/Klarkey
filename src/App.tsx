...
import {
  ...
  PasscodeSetupScreen,
} from "@/app/security-setup-screen";
...
function App() {
  ...
  const unlockWithPassword = usePaletteStore(
    (state) => state.unlockWithPassword,
  );
  const verifyPasscode = usePaletteStore((state) => state.verifyPasscode);
  const openSetPasscodePage = usePaletteStore(
    (state) => state.openSetPasscodePage,
  );
  const openSetMasterPasswordPage = usePaletteStore(
    (state) => state.openSetMasterPasswordPage,
  );
  const openConfirmPasscodeRemovalPage = usePaletteStore(
    (state) => state.openConfirmPasscodeRemovalPage,
  );
  const submitSetPasscode = usePaletteStore((state) => state.submitSetPasscode);
  const submitSetMasterPassword = usePaletteStore(
    (state) => state.submitSetMasterPassword,
  );
  const confirmPasscodeRemoval = usePaletteStore(
    (state) => state.confirmPasscodeRemoval,
  );
  ...
  const openRecoveryCodesPage = usePaletteStore(
    (state) => state.openRecoveryCodesPage,
  );
  const submitRecoveryCodes = usePaletteStore(
    (state) => state.submitRecoveryCodes,
  );
  const updateRecoveryCodesInPlace = usePaletteStore(
    (state) => state.updateRecoveryCodesInPlace,
  );
  ...
  const createSeed = useMemo(
    () => ({
      ...
      itemName:
        (detailAction?.kind === "create-item"
          ? detailAction.subtitle
          : undefined) ||
        query.itemQuery ||
        "",
  ...
  const footerOtp =
    execution?.title === "Current OTP" ? activeDetailItem?.otp : undefined;
    ...
    >
      <div className="flex items-center gap-4 px-5 pt-4 pb-3">
        {page === "home" ? (
          ...
          <HeaderRow
            ...
            subtitle={
              activeDetailItem?.itemName || createSeed.itemName || undefined
            }
            onBack={() => void goBackOrClose()}
            showIcon={false}
          />
      ...
      {page === "settings" ? (
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <SettingsPage
              ...
              onCycleClipboardClear={() =>
                void updateSettings({
                  clearClipboardSeconds: nextClipboardSeconds(
                    (settings ?? DEFAULT_SETTINGS).clearClipboardSeconds,
                  ),
                })
              }
              onToggleStartup={() =>
                void updateSettings({
                  launchOnStartup: !(settings ?? DEFAULT_SETTINGS)
                    .launchOnStartup,
                })
              }
              onToggleAutoOpenMenu={() =>
                void updateSettings({
                  browserAutoOpenMenu: !(settings ?? DEFAULT_SETTINGS)
                    .browserAutoOpenMenu,
                })
              }
              onToggleAutoSubmit={() =>
                void updateSettings({
                  browserAutoSubmitLogin: !(settings ?? DEFAULT_SETTINGS)
                    .browserAutoSubmitLogin,
                })
              }
              onToggleSavePrompts={() =>
                void updateSettings({
                  browserSavePrompts: !(settings ?? DEFAULT_SETTINGS)
                    .browserSavePrompts,
                })
              }
              onCycleAutoLock={() =>
                void updateSettings({
                  autoLockMinutes: nextAutoLockMinutes(
                    (settings ?? DEFAULT_SETTINGS).autoLockMinutes,
                  ),
                })
              }
              onTogglePasscode={() =>
              ...
              onToggleSshAgent={() =>
                void updateSettings({
                  sshAgentEnabled: !(settings ?? DEFAULT_SETTINGS)
                    .sshAgentEnabled,
                })
              }
        ...
        <ItemFormPage
          ...
          loading={formLoading}
          initialValue={createFormValues(
            formMode === "edit" ? formItemType : createItemType,
            formMode === "edit" ? activeDetailItem : createSeed,
          )}
        ...
        <>
          <div className="px-5 py-3 text-[13px] text-white/34">
            {query.raw ? "Results" : "Suggestions"}
          </div>
          <div
          ...
          >
            <div className="space-y-1">
              ...
              {isLoadingMore ? (
                <div className="px-3 py-3 text-[13px] text-white/34">
                  Loading more…
                </div>
              ) : null}
              {!isLoadingMore && hasMoreResults ? (
                <div className="px-3 py-3 text-[13px] text-white/24">
                  Scroll for more
                </div>
              ) : null}
            </div>
