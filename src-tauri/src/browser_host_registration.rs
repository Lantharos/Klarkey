...
pub(crate) fn ensure_native_host_registration(app: &AppHandle) -> Result<(), String> {
    let executable = env::current_exe()
        .map_err(|_| String::from("Could not resolve the Klarkey executable."))?;
    if cfg!(target_os = "windows") {
        ensure_windows_registration(app, &executable)
    } else {
...
#[cfg(target_os = "windows")]
fn ensure_windows_registration(app: &AppHandle, executable: &Path) -> Result<(), String> {
    let manifest_dir = app
        .path()
        .app_local_data_dir()
        .map_err(|_| String::from("Could not resolve the native host folder."))?
...
fn ensure_windows_registration(_app: &AppHandle, _executable: &Path) -> Result<(), String> {
    Ok(())
}
