use crate::{app_context::AppContext, secret_hash, secure_state, system_auth};
...
pub fn pick_import_file(ctx: &AppContext, format: String) -> Option<String> {
    ...
    dialog
        ...
        .pick_file()
        .map(|path| grant_path(path, &ctx.file_access().readable))
}

pub fn pick_export_file(ctx: &AppContext, format: String) -> Option<String> {
    pick_export_file_inner(ctx, format)
}

fn pick_export_file_inner(ctx: &AppContext, format: String) -> Option<String> {
    ...
    dialog
        .save_file()
        .map(|path| grant_path(path, &ctx.file_access().writable))
}

pub fn read_text_file(ctx: &AppContext, path: String) -> Result<String, String> {
    let path = consume_path_grant(path, &ctx.file_access().readable)?;
    fs::read_to_string(path).map_err(|_| String::from("Could not read the selected file."))
}

pub fn read_binary_file(ctx: &AppContext, path: String) -> Result<String, String> {
    let path = consume_path_grant(path, &ctx.file_access().readable)?;
...
pub fn write_text_file(ctx: &AppContext, path: String, contents: String) -> Result<(), String> {
    let path = consume_path_grant(path, &ctx.file_access().writable)?;
    fs::write(path, contents).map_err(|_| String::from("Could not write the selected file."))
}

pub fn load_vault_state(ctx: &AppContext) -> Result<Option<String>, String> {
    let path = vault_state_path(ctx)?;
...
pub fn lock_vault_metadata(ctx: &AppContext) -> Result<(), String> {
    let path = vault_state_path(ctx)?;
    system_auth::clear_cached_vault_key();
    secure_state::mark_vault_locked(&path)
}

pub fn save_vault_state(ctx: &AppContext, contents: String) -> Result<(), String> {
    let path = vault_state_path(ctx)?;
    if secure_state::read_vault_metadata(&path)?.is_some_and(|metadata| metadata.locked) {
...
pub fn unlock_vault_with_system(ctx: &AppContext, reason: String, strict: Option<bool>) -> Value {
    let auth = system_auth::unlock(reason, strict.unwrap_or(false));
    if !auth
        .get("success")
    ...
    unlock_state(ctx).unwrap_or_else(error_unlock_result)
}

pub fn unlock_vault_with_secret(ctx: &AppContext, kind: String, secret: String) -> Value {
    if let Some(message) = unlock_rate_limit_message() {
        return json!({ "success": false, "message": message });
    }
    let result = verify_secret_and_unlock(ctx, &kind, &secret);
    if result
        .get("success")
        .and_then(Value::as_bool)
        .unwrap_or(false)
...
pub fn reset_vault_state(ctx: &AppContext) -> Result<(), String> {
    let path = vault_state_path(ctx)?;
...
fn vault_state_path(ctx: &AppContext) -> Result<PathBuf, String> {
    ctx.vault_state_path()
}

fn verify_secret_and_unlock(ctx: &AppContext, kind: &str, secret: &str) -> Value {
    let path = match vault_state_path(ctx) {
        Ok(path) => path,
...
fn unlock_state(ctx: &AppContext) -> Result<Value, String> {
    let path = vault_state_path(ctx)?;
