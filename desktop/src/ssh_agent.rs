use crate::{app_context::AppContext, secure_state};
use rsa::{
    pkcs1v15::SigningKey,
    sha2::{Sha256, Sha512},
...
pub(crate) fn ssh_agent_apply(ctx: &AppContext, enabled: bool) -> Value {
    if enabled {
        match start(ctx) {
...
pub(crate) fn ssh_agent_status(ctx: &AppContext) -> Value {
    let metadata_enabled = vault_state_path(ctx)
        .ok()
...
pub(crate) fn start_from_metadata(ctx: &AppContext) {
    let enabled = vault_state_path(ctx)
        .ok()
        .and_then(|path| secure_state::read_vault_metadata(&path).ok().flatten())
        .is_some_and(|metadata| metadata.ssh_agent_enabled);
    if enabled {
...
fn start(ctx: &AppContext) -> Result<String, String> {
    let mut task = ctx
        .ssh_agent()
        .task
        .lock()
    ...
    let handle = spawn_listener(ctx.clone(), binding)?;
...
fn stop(ctx: &AppContext) {
    let Ok(mut task) = ctx.ssh_agent().task.lock() else {
        return;
    };
...
impl Session for KlarkeySshAgent {
    ...
    async fn sign(&mut self, request: SignRequest) -> Result<Signature, AgentError> {
        let keys = load_agent_keys(&self.ctx).map_err(|_| AgentError::Failure)?;
...
fn load_agent_keys(ctx: &AppContext) -> Result<Vec<AgentKey>, String> {
...
fn vault_state_path(ctx: &AppContext) -> Result<PathBuf, String> {
    ctx.data_dir().map(|path| path.join(STATE_FILE))
}
...
fn spawn_listener(
    ...
    Ok(runtime.spawn(async move {
        if let Err(error) = listen(listener, KlarkeySshAgent { ctx }).await {
